...
void OcctQmlViewer::showAxes(bool show)
{
  QMutexLocker lock(&m_mutex);
  m_showAxes = show;
...
void OcctQmlViewer::addTestBox()
{
  // Создаем простой куб для тестирования
  TopoDS_Shape box = BRepPrimAPI_MakeBox(50.0, 50.0, 50.0).Shape();
  addShape(box, AIS_Shaded);
}

void OcctQmlViewer::addTestCylinder()
{
  // Создаем цилиндр для тестирования
  TopoDS_Shape cylinder = BRepPrimAPI_MakeCylinder(25.0, 50.0).Shape();
  addShape(cylinder, AIS_Shaded);
}

void OcctQmlViewer::addTestSphere()
{
  // Создаем сферу для тестирования
  TopoDS_Shape sphere = BRepPrimAPI_MakeSphere(30.0).Shape();
  addShape(sphere, AIS_Shaded);
}
                                ...
                                QString& outGlInfo) const
{
  ...
  outShowAxes           = m_showAxes;       const_cast<bool&>(m_axesRequested) = false;
  outDatum              = m_datum;
  outGlInfo             = m_glInfo;
}
...
void OcctQmlViewer::RendererImpl::synchronize(QQuickFramebufferObject* item)
{
  ...
  v->takePending(m_toAdd, m_doClear, m_doReset, m_resetDistance, m_doFitAll, m_doClickSelect, m_showAxes, m_datum, m_glInfo);                                                                                                       
...
void OcctQmlViewer::RendererImpl::createAxes()
{
  if (m_context.IsNull()) return;

  // Create X axis (red)
  TopoDS_Edge xAxis = BRepBuilderAPI_MakeEdge(gp_Pnt(0, 0, 0), gp_Pnt(100, 0, 0)).Edge();
  m_axisX = new AIS_Shape(xAxis);
  Handle(Prs3d_Drawer) xDrawer = new Prs3d_Drawer();
  Handle(Prs3d_LineAspect) xLineAspect = new Prs3d_LineAspect(Quantity_Color(1.0, 0.0, 0.0, Quantity_TOC_sRGB), Aspect_TOL_SOLID, 3.0);
  xDrawer->SetWireAspect(xLineAspect);
  m_axisX->SetAttributes(xDrawer);
  m_axisX->SetDisplayMode(AIS_WireFrame);
  m_axisX->SetAutoHilight(false);

  // Create Y axis (green)
  TopoDS_Edge yAxis = BRepBuilderAPI_MakeEdge(gp_Pnt(0, 0, 0), gp_Pnt(0, 100, 0)).Edge();
  m_axisY = new AIS_Shape(yAxis);
  Handle(Prs3d_Drawer) yDrawer = new Prs3d_Drawer();
  Handle(Prs3d_LineAspect) yLineAspect = new Prs3d_LineAspect(Quantity_Color(0.0, 1.0, 0.0, Quantity_TOC_sRGB), Aspect_TOL_SOLID, 3.0);
  yDrawer->SetWireAspect(yLineAspect);
  m_axisY->SetAttributes(yDrawer);
  m_axisY->SetDisplayMode(AIS_WireFrame);
  m_axisY->SetAutoHilight(false);

  // Create Z axis (blue)
  TopoDS_Edge zAxis = BRepBuilderAPI_MakeEdge(gp_Pnt(0, 0, 0), gp_Pnt(0, 0, 100)).Edge();
  m_axisZ = new AIS_Shape(zAxis);
  Handle(Prs3d_Drawer) zDrawer = new Prs3d_Drawer();
  Handle(Prs3d_LineAspect) zLineAspect = new Prs3d_LineAspect(Quantity_Color(0.0, 0.0, 1.0, Quantity_TOC_sRGB), Aspect_TOL_SOLID, 3.0);
  zDrawer->SetWireAspect(zLineAspect);
  m_axisZ->SetAttributes(zDrawer);
  m_axisZ->SetDisplayMode(AIS_WireFrame);
  m_axisZ->SetAutoHilight(false);
